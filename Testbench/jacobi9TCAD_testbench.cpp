...
*/
	for (int i=0; i<HEIGHT;i++){
		for(int j=0; j<WIDTH; j++){
			mult=mat(rand());
			//A[i][j]= mult*mat(rand())/mat(RAND_MAX); //Initialize A matrix as random floats
	...
	for (int i=0; i<HEIGHT; i++){
		...
		// Check HW result against SW
			...
			if (fabs(hw_result[i][j] - sw_result[i][j])>1e-2f) { //Just checking the Kernel results not the full size matrix with the HALO
